...
etichette_clusterizzazione = db.labels_

# Number of clusters in labels, ignoring noise if present.
n_clusters_ = len(set(etichette_clusterizzazione)) - (1 if -1 in etichette_clusterizzazione else 0)
n_noise_ = list(etichette_clusterizzazione).count(-1)
...
print(f"Homogeneity: {metrics.homogeneity_score(labels_true, etichette_clusterizzazione):.3f}")
print(f"Completeness: {metrics.completeness_score(labels_true, etichette_clusterizzazione):.3f}")
print(f"V-measure: {metrics.v_measure_score(labels_true, etichette_clusterizzazione):.3f}")
print(f"Adjusted Rand Index: {metrics.adjusted_rand_score(labels_true, etichette_clusterizzazione):.3f}")
print(
    "Adjusted Mutual Information:"
    f" {metrics.adjusted_mutual_info_score(labels_true, etichette_clusterizzazione):.3f}"
)
print(f"Silhouette Coefficient: {metrics.silhouette_score(X, etichette_clusterizzazione):.3f}")
...
unique_labels = set(etichette_clusterizzazione)
core_samples_mask = np.zeros_like(etichette_clusterizzazione, dtype=bool)
...
for k, col in zip(unique_labels, colors):
    ...
    class_member_mask = etichette_clusterizzazione == k
