...
my_list2 [0] = -1                                         #così sto cambiando il valore del primo elemento della lista da 1 a -1, rendendo le liste mutabili

try:                                                      #esempio di gestione delle eccezioni con il CS try, dove si risponde con except
    my_list[100]                                          #qui si genera un'eccezione del tipo index out of range, ma che può essere gestita
except Exception as ex:                                   #si crea la variabile di eccezione ex che richiama la Exception, in modo da poterla gestire
    print(str(ex))                                        # il programma non si è bloccato per via della gestione dell'eccezione
print("done")
...
"""
quello che normalmente uno pensa circa la copia delle funzioni, ma in realtà non è così:
#in realtà sto solamente copiando gli ultimi valori registrati in memoria per quella lista, andando a copiare il riferimento di memoria
#creazione delle copie scorrelate dal codice:   dalla debug console pongo la #!my_copy_my_list4=list(my_list4)
#list è un costruttore, ossia crea un nuovo oggetto da una lista preesistente, per dare poi vita ad una nuova lista, che ha vita propria rispetto alla
