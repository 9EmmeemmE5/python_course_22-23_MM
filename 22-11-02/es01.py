...
# un file binario è composto da caratteri che non hanno molto senso se non decodificati con un apposito algoritmo
...
# se avessi un numero molto lungo con decine di migliaia di cifre decimali, se si salva il file in formato testuale, la sua dimensione sarà molo grande,
# mentre in binario sarà molto più piccola, ma con l'handicap di andare a capire come sono stati codificati i file binari in modo da poterli poi decodificare per l' utlizzo.
...
# che è quello che è stato usato per generare la combinazione in binario dello stesso (il prof ha fatto un check per osservare la dimensione); il rapporto di dimensione tra 
# binario e testuale è di 1:4.

# se chiedo di aprire un file con la funzione open, allora dovrò anche chiuderlo per poterne permettere l'uso ad altri utenti,
...
def main():
    """Entry-point"""
    #apertura di un file...
    my_file = open("esempio_open_funct.txt")    #in grigetto perche non vi è accesso, inoltre rimane aperto, non viene chiuso
    ...
    while True:
        ...
        print(my_line)
    my_file.close()
    my_line = my_file.readline() #va in eccezione perché sto operando su un file chiuso, distruggendo le connessioni logiche con lo stesso, quindi #!CHIUDERE SEMPRE IL FILE DOPO
