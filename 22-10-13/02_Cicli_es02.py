...
"""
scegliere due punti A e B tali per cui f(A) ed f(B) siano uno >0 ed uno <0: si può dimostrare che se la funzione presenta 
un solo 0, allora è possibile andare, iterativamente, a cercare la soluzione per approssimazioni successive, partendo ad esempio dal punto mediano,
ossia (B-A)/2 + A, andando a verificare il segno della funzione (positiva o negativa) sul punto mediano; se è positivo ho un nuovo a, mentre se è negativa
...
Si usa un while perche continua fino a che non si è arrivato al risultato desiderato, piuttosto che un for
"""

#creare la funzione matematica monovariabile...
#x^3 - 4*x^2 + 6*x - 24.5 = y è la funzione da analizzare
...
def my_poly(x_value = 0):                                                      #definisco la funzione con il def, che è in blu scuro perche è riservato per python
    """
    la funzione calcola il valore di un polinomio di 3° grado...
    """
    #return func_value = x_value ** 3 - 4 * x_value ** 2 + 6 * x_value - 24.5   #da errore solo con il return, che è stato inserito perche non è stato richiamato l'assegnamento, 
...
#! ho inserito un'assegnazione a x_value pari a 0.0 (l'integer non è ammesso) col doppio "="
...
print(my_value)                                              #printo con l'input dell'utente: se il numero è grande la convergenza è lenta
