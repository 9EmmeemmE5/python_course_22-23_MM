...
def main():
    ...
    plt.hist(roi_img.ravel(),256,[0,256])  #da rivedere, sembra non corretto causa rgb in bw?
    plt.xlim([0,256])
    plt.show()
    ...
    Questo perche l'algoritmo si applica ad una serie di misurazioni e non ad una singola: occorre tradurre la 
    matrice di immagine in un vettore di temperature, per poi andare a clusterizzare i valori di temperatura, per
    poi andare a ritrasformare il vettore clusterizzato in immagine.
    L'algoreitmo richiede come parametro importante il numero di cluster e poi si esegue il fit; nel nostro caso si hanno
    n misurazioni monodimensionali.
    Il fatto di aver fatto un fit a monte permette di essere riutilizzato con molte predict sulla base di un solo training iniziale,
    quindi i dati si usano a monte per addestrare, per poi andare ad essere usati a valle per l'analisi con le prediction
    """
    ...
    #! aggiungi la parte di predict che il prof fa vedere sulla debug console, vedi anche se vuoi https://www.datacamp.com/courses/unsupervised-learning-in-python per fare l'unsupervised machine data learning che, in quanto studenti univpm, non si paga, a mezzo di github student pack
