...
# possibile editare il breakpoint: ad esempio yrs == 5, il breakpoint si modifica al count 5,
# divenendo un conditional bkpointfacendo iterare il ciclo while,
# a meno di breakpoint standard messi prima, fino al 5° conteggio
...
while amount_actual < amount_target:
    if(years > MAX_YEARS):
        break
    # attenzione a dove si mettono le istruzioni di break, perche se fosse stato messo sotto
    # dopo amount_actual, allora avrebbe fatto il 13° giro
    years += 1
    amount_actual = amount_actual * (1 + growing_rate)  
print(f"il capitale dopo {years} anni e': {amount_actual:.2f}")